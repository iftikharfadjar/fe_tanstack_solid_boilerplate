...
function useQuery(args) {
  ...
  var [state, setState] = React.useState(() => [source, computeNextState(initialState, deferDispatch(() => getSnapshot(source, suspense))), deps]);
  var currentResult = state[1];
  if (source !== state[0] && hasDepsChanged(state[2], deps)) {
    setState([source, currentResult = computeNextState(state[1], deferDispatch(() => getSnapshot(source, suspense))), deps]);
  }
  React.useEffect(() => {
    var source = state[0];
